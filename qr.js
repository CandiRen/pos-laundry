 ...
 * MIT License
 *
 * The library is slightly trimmed to fit LaundryPOS needs (canvas rendering only).
 */
(function () {
  ...
  QRCode.prototype = {
    ...
    make: function () {
      if (this.typeNumber < 1) {
        this.typeNumber = 1; // auto sizing minimal support
      }
    ...
    setupTypeInfo: function (test, maskPattern) {
      ...
      for (var j = 0; j < 15; j++) {
        var modJ = !test && ((bits >> j) & 1) === 1;
        if (j < 8) {
          this.modules[8][this.moduleCount - j - 1] = modJ;
        } else if (j < 9) {
          this.modules[8][15 - j - 1 + 1] = modJ;
        } else {
          this.modules[8][15 - j - 1] = modJ;
        }
      }
  ...
  QRCanvasRenderer.prototype.draw = function () {
    var ctx = this.canvas.getContext("2d");
    var count = this.qr.getModuleCount();
    var size = this.size;
    var margin = this.margin;
    var tile = (size - margin * 2) / count;
    ...
    for (var row = 0; row < count; row++) {
      for (var col = 0; col < count; col++) {
        if (this.qr.isDark(row, col)) {
          var x = Math.round(col * tile + margin);
          var y = Math.round(row * tile + margin);
          var w = Math.ceil((col + 1) * tile + margin) - x;
          var h = Math.ceil((row + 1) * tile + margin) - y;
  ...
  function generateQRCode(canvas, text, opts) {
    opts = opts || {};
    var qr = new QRCode(4, QRErrorCorrectLevel.M);
    qr.addData(text);
    qr.make();
    var renderer = new QRCanvasRenderer({
      canvas: canvas,
      qr: qr,
      size: opts.size || 200,
